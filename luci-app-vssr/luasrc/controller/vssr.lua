...
function index()
	...
	local page = entry({'admin', 'vpn', 'vssr'}, alias('admin', 'vpn', 'vssr', 'client'), _('Hello World'), 0) -- 首页
	page.dependent = true
	...
	entry({'admin', 'vpn', 'vssr', 'server'}, arcombine(cbi('vssr/server'), cbi('vssr/server-config')), _('SSR Server'), 20).leaf = true -- 服务端
	entry({'admin', 'vpn', 'vssr', 'log'}, cbi('vssr/log'), _('Log'), 30).leaf = true -- 日志
